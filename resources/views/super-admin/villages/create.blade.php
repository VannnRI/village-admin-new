...
@section('content')
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Tambah Desa</h1>
        <p class="text-gray-600">Buat desa baru dalam platform</p>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('super-admin.villages.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <textarea name="vision" id="vision" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500">{{ old('vision') }}</textarea>
                    @error('vision')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="mission" id="mission" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500">{{ old('mission') }}</textarea>
                    @error('mission')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-8 flex justify-end space-x-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Simpan Desa
                </button>
            </div>
        </form>
    </div>
