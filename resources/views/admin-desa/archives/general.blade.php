...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form class="space-y-4" method="POST" action="{{ route('admin-desa.archives.archives.general.store') }}" enctype="multipart/form-data">
            @csrf
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 cursor-pointer" @click="setDoc({
                        id: {{ $document->id }},
                        title: '{{ addslashes($document->title) }}',
                        category: '{{ addslashes($document->category) }}',
                        description: `{{ addslashes($document->description) }}`,
                        file: '{{ addslashes($document->file) }}',
                        size: '{{ $document->size ? number_format($document->size / 1048576, 2) . ' MB' : '-' }}',
                        date: '{{ $document->date ? \Carbon\Carbon::parse($document->date)->format('d/m/Y') : '-' }}',
                    })">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            ...
                            <form action="{{ route('admin-desa.archives.archives.general.destroy', $document->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus dokumen ini?')">
                                    @csrf
                                    @method('DELETE')
